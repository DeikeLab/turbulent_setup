...
int main(int argc, char *argv[]) {
  ...
  Ustar   = 0; // Pick a fixed value
  rho1    = 1.;
  rho2    = rho1*RHO_RATIO;
  g_      = k_*sq(c_)/( 1.0+(rho1-rho2)/(rho1*BO) ); // tune g_ 
  f.sigma = g_*(rho1-rho2)/(BO*sq(k_));
  G.y     = -g_;
  RE      = 25000.;//rho1*c_*(2*pi/k_)/mu1; // RE now becomes a dependent Non-dim number on c
  mu1     = rho1*sqrt(g_*k_)*(2*pi/k_)/RE;
  mu2     = mu1*MU_RATIO; 
     ...
     // double ytau = (mu2/rho2)/Ustar;
      do {
        ...
        foreach() {
          ...
          u.y[] = 0.0*(1.0-f[]);//(-1.0*gxz(z_n,x_n)*dfy(y_n)*Ubulk_ex*1.5)*(1.0-f[]);
          u.z[] = 0.0*(1.0-f[]);//(+1.0*fy(y_n)*dgxz(z_n,x_n)*Ubulk_ex*1.5)*(1.0-f[]);
        }
        boundary ((scalar *){u});
      }
#if TREE
...
//event movies (i += 2) {
  ...
  view (fov = 27.5,  
        theta = pi/4.0, phi = pi/50.0, psi = 0.0, ty = -0.5,
	//width = 1300, height = 1300, bg = {1,1,1}, samples = 4);
	width = 800, height = 800, bg = {1,1,1}, samples = 4);
  box(false, lc = {1,1,1}, lw = 0.1);
  draw_vof ("f", color = "u.x");
  squares ("u.x", linear = true, n = {0,0,1}, alpha = -L0/2.0);
...
#endif
          */
          pr_m += ar*my_interpolation(p_a, xc, yc, zc);
	...
	if (point.level > 0) {
	  ...
	  uz_int  = 0.;
          ...
          */
          ...
          pr_int  = my_interpolation(p_a, xc, yc, zc);
