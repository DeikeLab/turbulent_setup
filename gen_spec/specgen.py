...
def spectrum_PM_piece (P, kp, k_cf, kmod):
    C = spectrum_JONSWAP(P,kp,k_cf)/(k_cf)**(-15.0/4.0);
    if kmod < k_cf:
      F_kmod = P*kmod**(-3)*np.exp(-1.25*(kp/kmod)**2);
    else: 
      C = spectrum_JONSWAP(P,kp,k_cf)/(k_cf)**(-15.0/4.0);
      F_kmod = C*kmod**(-15.0/4.0);
    return F_kmod
...
def spectrum_gen_linear(shape, N_mode, N_power, L):
    ...
    kmod = np.linspace(2*np.pi/L,1.42*100*2*np.pi/L,N_kmod) # Change 
    #print(kmod.min());
    #print(kmod.max());
    ...
    F_kmod = np.zeros([len(kmod)]);
    for i in range(len(kmod)):
        F_kmod[i] = shape(kmod[i]);
    #
