...
event start (t = RELEASETIME) {
    fprintf(stderr, "Releasing waves at t=%g\n", t);
    fraction (f, WaveProfile(x,z)-y);
    scalar Phi[];
    foreach() {
        Phi[] += phi(x, y-h_);
    ...
    foreach() {
        // Calculate the velocities from the gradient of Phi
        //foreach_dimension()
        u.x[] += (Phi[1] - Phi[-1])/(2.0*Delta) * f[];
        u.y[] += (Phi[0,1] - Phi[0,-1])/(2.0*Delta) * f[]; // si pongo foreach_dimension lo hace directamente para cada dimension
    }
    boundary ((scalar *){u});
    //boundary ({Phi});
    ...
    //while (adapt_wavelet ({f, u},
                          //(double[]){femax,100.*uemax,100.*uemax,100.*uemax}, MAXLEVEL, 5).nf); // if not adapting anymore, return zero
    while (0);
#else
    while (0);
#endif
...
event movies (t = RELEASETIME; t <= T0_*end_sim; t += 0.001) {
//event movies (i += 2) {
